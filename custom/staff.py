            ...
            try:
                if interaction.guild.get_member(i).top_role > interaction.guild.get_member(selfBot.user.id).top_role:
                    ...
                    e.add_field(
                        ...
                        value=selfBot.get_user(i).mention
                    )
            except:
                user = await selfBot.fetch_user(i)
                e.add_field(
                    name=f"Membro a banir {e.fields.__len__() + 1}",
                    value= user.mention
                )
    ...
    if interaction.guild.get_role(configData["roles"]["staff"]["asmodeus"]) in interaction.user.roles \
        ...
        match e.title:
            case "Banimento":
                ...
                for ef in e.fields:
                    member = await selfBot.fetch_user(int(ef.value.strip("<@>")))
                    ...
                    await interaction.guild.ban(member,delete_message_seconds=86400, reason="Acumulo de advertencia")
                    embedsFinal.append(
                        discord.Embed(
                            title="Banimento",
                            colour=discord.Colour.red()
